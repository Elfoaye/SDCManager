...
// node_modules/@tauri-apps/plugin-window/node_modules/@tauri-apps/api/chunk-WVIOQTRJ.js
var f = Object.defineProperty;
var g = (a2, b) => {
  for (var c2 in b) f(a2, c2, { get: b[c2], enumerable: true });
};
var e = (a2, b, c2) => {
  if (!b.has(a2)) throw TypeError("Cannot " + c2);
};
var h = (a2, b, c2) => (e(a2, b, "read from private field"), c2 ? c2.call(a2) : b.get(a2));
var i = (a2, b, c2) => {
  if (b.has(a2)) throw TypeError("Cannot add the same private member more than once");
  b instanceof WeakSet ? b.add(a2) : b.set(a2, c2);
};
var j = (a2, b, c2, d) => (e(a2, b, "write to private field"), d ? d.call(a2, c2) : b.set(a2, c2), c2);

// node_modules/@tauri-apps/plugin-window/node_modules/@tauri-apps/api/chunk-VMGITBBQ.js
var h2 = {};
g(h2, { Channel: () => o, PluginListener: () => a, addPluginListener: () => m, convertFileSrc: () => y, invoke: () => u, transformCallback: () => s });
function f2() {
  return window.crypto.getRandomValues(new Uint32Array(1))[0];
}
function s(n, e2 = false) {
  let t = f2(), r = `_${t}`;
  return Object.defineProperty(window, r, { value: (c2) => (e2 && Reflect.deleteProperty(window, r), n?.(c2)), writable: false, configurable: true }), t;
}
var i2;
var o = class {
  constructor() {
...
var a = class {
  constructor(e2, t, r) {
    this.plugin = e2, this.event = t, this.channelId = r;
  }
  async unregister() {
    return u(`plugin:${this.plugin}|remove_listener`, { event: this.event, channelId: this.channelId });
  }
};
async function m(n, e2, t) {
  let r = new o();
  return r.onmessage = t, u(`plugin:${n}|register_listener`, { event: e2, handler: r }).then(() => new a(n, e2, r.id));
}
async function u(n, e2 = {}, t) {
  return new Promise((r, c2) => {
    let g2 = s((d) => {
      r(d), Reflect.deleteProperty(window, `_${_}`);
    }, true), _ = s((d) => {
      c2(d), Reflect.deleteProperty(window, `_${g2}`);
    }, true);
    window.__TAURI_IPC__({ cmd: n, callback: g2, error: _, payload: e2, options: t });
  });
}
function y(n, e2 = "asset") {
  return window.__TAURI__.convertFileSrc(n, e2);
}

// node_modules/@tauri-apps/plugin-window/node_modules/@tauri-apps/api/chunk-4EPV3FVZ.js
var E = {};
g(E, { TauriEvent: () => l, emit: () => D, listen: () => c, once: () => W });
var l = ((e2) => (e2.WINDOW_RESIZED = "tauri://resize", e2.WINDOW_MOVED = "tauri://move", e2.WINDOW_CLOSE_REQUESTED = "tauri://close-requested", e2.WINDOW_CREATED = "tauri://window-created", e2.WINDOW_DESTROYED = "tauri://destroyed", e2.WINDOW_FOCUS = "tauri://focus", e2.WINDOW_BLUR = "tauri://blur", e2.WINDOW_SCALE_FACTOR_CHANGED = "tauri://scale-change", e2.WINDOW_THEME_CHANGED = "tauri://theme-changed", e2.WINDOW_FILE_DROP = "tauri://file-drop", e2.WINDOW_FILE_DROP_HOVER = "tauri://file-drop-hover", e2.WINDOW_FILE_DROP_CANCELLED = "tauri://file-drop-cancelled", e2.MENU = "tauri://menu", e2))(l || {});
async function s2(n, t) {
  await u("plugin:event|unlisten", { event: n, eventId: t });
}
async function c(n, t, i3) {
  return u("plugin:event|listen", { event: n, windowLabel: i3?.target, handler: s(t) }).then((r) => async () => s2(n, r));
}
async function W(n, t, i3) {
  return c(n, (r) => {
    t(r), s2(n, r.id).catch(() => {
    });
  }, i3);
}
async function D(n, t, i3) {
  await u("plugin:event|emit", { event: n, windowLabel: i3?.target, payload: t });
}

// node_modules/@tauri-apps/plugin-window/dist-js/index.mjs
...
var Window = class _Window {
  ...
  constructor(label, options = {}) {
    ...
    if (!(options === null || options === void 0 ? void 0 : options.skip)) {
      window.__TAURI_INVOKE__("plugin:window|create", {
        options: {
          ...options,
          label
        }
  ...
  async listen(event, handler) {
    ...
    return c(event, handler, { target: this.label });
  }
  /**
  ...
  async once(event, handler) {
    ...
    return W(event, handler, { target: this.label });
  }
  /**
  ...
  async emit(event, payload) {
    ...
    return D(event, payload, { target: this.label });
  }
  /** @ignore */
   ...
   *  */
  async innerPosition() {
    return window.__TAURI_INVOKE__("plugin:window|inner_position", {
      label: this.label
    }).then(({ x, y: y2 }) => new PhysicalPosition(x, y2));
  }
  /**
   ...
   *  */
  async outerPosition() {
    return window.__TAURI_INVOKE__("plugin:window|outer_position", {
      label: this.label
    }).then(({ x, y: y2 }) => new PhysicalPosition(x, y2));
  }
  /**
   ...
   */
  async innerSize() {
    return window.__TAURI_INVOKE__("plugin:window|inner_size", {
      label: this.label
   ...
   */
  async outerSize() {
    return window.__TAURI_INVOKE__("plugin:window|outer_size", {
      label: this.label
    }).then(({ width, height }) => new PhysicalSize(width, height));
  }
  /**
  ...
  async onResized(handler) {
    return this.listen(l.WINDOW_RESIZED, (e2) => {
  ...
  async onMoved(handler) {
    return this.listen(l.WINDOW_MOVED, (e2) => {
  ...
  async onCloseRequested(handler) {
    return this.listen(l.WINDOW_CLOSE_REQUESTED, (event) => {
  ...
  async onFocusChanged(handler) {
    const unlistenFocus = await this.listen(l.WINDOW_FOCUS, (event) => {
      handler({ ...event, payload: true });
    });
    const unlistenBlur = await this.listen(l.WINDOW_BLUR, (event) => {
  ...
  async onScaleChanged(handler) {
    return this.listen(l.WINDOW_SCALE_FACTOR_CHANGED, handler);
  }
  /**
  ...
  async onMenuClicked(handler) {
    return this.listen(l.MENU, handler);
  }
  /**
  ...
  async onFileDropEvent(handler) {
    const unlistenFileDrop = await this.listen(l.WINDOW_FILE_DROP, (event) => {
      handler({ ...event, payload: { type: "drop", paths: event.payload } });
    });
    const unlistenFileHover = await this.listen(l.WINDOW_FILE_DROP_HOVER, (event) => {
      handler({ ...event, payload: { type: "hover", paths: event.payload } });
    });
    const unlistenCancel = await this.listen(l.WINDOW_FILE_DROP_CANCELLED, (event) => {
  ...
  async onThemeChanged(handler) {
    return this.listen(l.WINDOW_THEME_CHANGED, handler);
  }
};
var Effect;
(function(Effect2) {
...
function mapMonitor(m2) {
  return m2 === null ? null : {
    ...
    position: mapPhysicalPosition(m2.position),
    size: mapPhysicalSize(m2.size)
  };
}
...
//# sourceMappingURL=@tauri-apps_plugin-window.js.map
