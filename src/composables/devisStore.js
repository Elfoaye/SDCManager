...
export const useDevisStore = defineStore('devis', () => {
    ...
    async function saveDevis(state) {
        ...
        const fullDevis = {
            ...
            devis: {
                ...
                durée: Number(devisInfos.value.duration) || 1,
                nb_tech: Number(utilitaries.value.techQty) || 0,
                taux_tech: Number(utilitaries.value.techRate) || 0,
                nb_km: Number(utilitaries.value.transportKm) || 0,
                taux_km: Number(utilitaries.value.transportRate) || 0,
                adhesion: utilitaries.value.membership,
                promo: Number(utilitaries.value.discountEuro) || 0,
                etat: state
            },
    ...
    async function loadDocument(document) {
        try {
            const fullDocument = document.facture ? await invoke('load_facture', { factureId: Number(document.id) }) :
            await invoke('load_devis', { devisId: Number(document.id) });
